 ...
 */ 
export function toMatTypeName(type: number): MatTypes | undefined {
...
export default function (newCv: typeof openCV): void {
    ...
    cv = newCv;
    // @ts-ignore
    cv.getVersion = getVersion;
    // @ts-ignore
    cv.getVersionString = getVersionString;
    // @ts-ignore
    cv.toMatTypeName = toMatTypeName;
    // @ts-ignore
    cv.getScoreMax = getScoreMax;
    // @ts-ignore
    cv.dropOverlappingZone = dropOverlappingZone;
}
